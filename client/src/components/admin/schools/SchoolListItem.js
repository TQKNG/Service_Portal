...
import { setSchool } from '../../../actions/admin';
import ToolTipComp from '../../layouts/ToolTip';

const SchoolsListItem = ({ school, setSchool }) => {
  const [edit, setEdit] = useState(false);
  const [show, setShow] = useState(false);
  const { SchoolID, Name } = school;
  const hist = useHistory();
  return (
    <div className='admin-users-fields  d-flex align-items-center justify-content-around   p-2 border-bottom'>
      <div className='admin-schools-field text-truncate mr-1'>{SchoolID}</div>
      <div
...
export default connect(null, { setSchool })(SchoolsListItem);
