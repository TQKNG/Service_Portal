...
const AssessmentListItem = ({ assessment, getAssessment, user }) => {
  const [edit, setEdit] = useState(false);
  const [show, setShow] = useState(false);
  const { AssessmentID, Title, GradeID, Category, CategoryID } = assessment;
    ...
    <div className='admin-users-fields  d-flex align-items-center justify-content-around   p-2 border-bottom'>
      <div
        ...
        onClick={() => {
          ...
          hist.push(
            `/admin/assessment/assessmentId=${AssessmentID}/categoryId=${CategoryID}`,
          );
        }}
        title={Title}
      >
      ...
      <div
        className='admin-assessments-field text-truncate mr-1 d-none d-sm-block'
        title={Category}
      >
        {Category}
      </div>
      ...
      <div className='admin-assessments-field  text-truncate  '>
        {user.UserTypeID === 5 && (
          ...
          <ToolTipComp
            myButton={  <svg
              ...
              onClick={() => {
                getAssessment(AssessmentID, CategoryID).then(() =>
...
export default connect(null, { getAssessment })(AssessmentListItem);
